...
void createBoneMap(std::vector<VertexData>& vertexData, aiMesh* mesh, const aiScene* scene) {
	...
	for (int j = 0; j < mesh->mNumBones; ++j) {
		...
		if (currentModel->BoneMap.find(boneName) == currentModel->BoneMap.end()) {

			// create bone struct for animation with id to reference vertex
			Bone newBone;
			newBone.m_ID = currentModel->m_BoneCounter;
			newBone.m_OffsetMatrix = AssimpGLMHelpers::ConvertMatrixToGLMFormat(mesh->mBones[j]->mOffsetMatrix);

			// put in bone map
			currentModel->BoneMap[boneName] = newBone;
			
			// set boneId for vertex and increment
			boneID = currentModel->m_BoneCounter;
			currentModel->m_BoneCounter++;
		}
		else {
			boneID = currentModel->BoneMap[boneName].m_ID;
		}
...
void processNode(aiNode* node, const aiScene* scene, Node rootNode)
{
	assert(node);

	rootNode.m_Name = node->mName.data;

	rootNode.m_Tansformation = AssimpGLMHelpers::ConvertMatrixToGLMFormat(node->mTransformation);

	rootNode.childrenCount = node->mNumChildren;
